...
public class UtilisateurDAO extends AbstractDAO<Utilisateur> {
    ...
    protected Utilisateur map(ResultSet resultSet) throws SQLException {
        ...
        StatutUtilisateur statut = getFactory().getStatutUtilisateurDAO().findById(resultSet.getLong("idStatutUtilisateur"));
        List<Role> roles = getFactory().getRoleDAO().findByUtilisateur(utilisateur.getId());
